...
log using PLAN604_PopulationProjection_2018-11-20, replace text
/********-*********-*********-*********-*********-*********-*********/
/* Description of Program                                           */
/********-*********-*********-*********-*********-*********-*********/
* program:    PLAN604_PopulationProjection_2018-11-20.do
...
* 				https://factfinder.census.gov/bkmk/table/1.0/en/ACS/17_1YR/S0101/0500000US48041
...
global state = "Texas"
global FIPSCounty = "48041"

* What years will be used for the population projections models?
* The base year varies depending on the community and the data available
* Each population model can have a different base year.
global base_year_share     = 2000
global base_year_linear    = 1970
global base_year_geometric = 1850
global base_year_parabolic = 1850
global base_year_logistic  = 1850

* The launch year is the last year with observed population data:
global launch_year = 2017

* The target year is in the future.
global target_year = 2050 // Projected Population Year

* This shift-share and share of growth methods require data for 
* a larger region - usually the state.

*  The population of the state or larger region in base and launch years
* For the shift-share and share of growth base year and launch year
*Source:
* https://factfinder.census.gov/bkmk/table/1.0/en/DEC/10_SF1/SF1DP1/0400000US48
*https://factfinder.census.gov/bkmk/table/1.0/en/ACS/17_1YR/S0101/0400000US48
global pop_large_region_base = 25145561 
global pop_large_region_launch = 28304596

*  The projected population of the state or larger region in target year
* Source of data will depend on state the county is in.
* Texas Demographic Center’s Texas Population Projections Tool
* http://txsdc.utsa.edu/Data/TPEPP/Projections/Tool
* Assuming migration rate of 1/2 2000-2010 migration rate
* For states without population projection data use
* https://wonder.cdc.gov/population-projections.html
global pop_large_region_target = 40502749
...
notes pop: Data Source: US Census Bureau 
...
replace year = ${launch_year} + _n - `current_number_of_rows' if year == .
...
* For the linear regression models (linear, geometric, parabolic, and logistic)
...
putdocx text ("5. ${target_year} Population Projections for ${countyname}, ${state}")

* Figure 3.6
twoway(line pop year, lwidth(thick)) ///
	  (scatter pop year, msymbol(S)) ///
	  if year < ${launch_year}, graphregion(color(white)) ylabel(, angle(0))
graph export "Pop_${countyname}_${state}_observed_pop.emf", replace
putdocx paragraph, halign(center)
putdocx image "Pop_${countyname}_${state}_observed_pop.emf"
putdocx paragraph, halign(center)
putdocx text ("Figure 5.1"), bold
...
putdocx text ("The Average Annual Absolute Change between ${base_year_share}-${launch_year} was `AAAC_text'")

* Average Annual Percent Change (AAPC)
local AAPC = (`pop_launch'/`pop_base')^(1/`n')-1
putdocx paragraph
local AAPC_text : display %6.3f `AAPC'
putdocx text ("The Average Annual Percent Change between ${base_year_share}-${launch_year} was `AAPC_text'")
...
putdocx text ("5.1 Share of Growth Method")

putdocx paragraph, halign(center)
putdocx text ("Table 5.1 "), bold
...
putdocx paragraph, halign(left) shading(yellow)
putdocx text ("Source: What is the data source for this table? ")
...
/********-*********-*********-*********-*********-*********-*********/
putdocx paragraph, style(Heading2)
putdocx text ("5.2 Shift-Share Method")
...
/********-*********-*********-*********-*********-*********-*********/
putdocx paragraph, style(Heading2)
putdocx text ("5.3 Linear Population Model")
...
putdocx text ("The y-intercept (year=${base_year_linear}) for the linear model is ${alpha_linear}")
...
putdocx paragraph, halign(center)
putdocx text ("Figure 5.3 "), bold
putdocx text ("The linear trend line for ${countyname}, ${state} population data")

* Save a high quality vector graphic to replace low quality png
graph export "Pop_${countyname}_${state}_linear.emf", replace
...
/********-*********-*********-*********-*********-*********-*********/
putdocx paragraph, style(Heading2)
putdocx text ("5.4 Geometric Population Model")
...
/********-*********-*********-*********-*********-*********-*********/
putdocx paragraph, style(Heading2)
putdocx text ("5.5 Parabolic Population Model")
...
/********-*********-*********-*********-*********-*********-*********/
putdocx paragraph, style(Heading2)
putdocx text ("5.6 Logistic Population Model")
putdocx paragraph
...
/********-*********-*********-*********-*********-*********-*********/
putdocx paragraph, style(Heading2)
putdocx text ("5.7 Comparison of Population Projections")

putdocx paragraph, halign(center)
putdocx text ("Table 5.2"), bold
...
putdocx save "PLAN604_PopulationProjection_2018-11-20_${countyname}_${state}_${target_year}.docx", replace

/********-*********-*********-*********-*********-*********-*********/
/* Figure to compare Predicted Population                           */
/********-*********-*********-*********-*********-*********-*********/
putdocx begin

* Comparison of all models of population projection
twoway (line pop year if year >= ${launch_year}-10 , lwidth(thick)) ///
	  (scatter sharegrowth year if year >= ${launch_year}, msymbol(+) ) ///
	  (scatter shiftshare year if year >= ${launch_year}, msymbol(X)) ///
	  (scatter pop_linear year if year >= ${launch_year}, msymbol(o)) ///
	  (scatter pop_geometric year if year >= ${launch_year}, msymbol(d)) ///
	  (scatter pop_parabolic year if year >= ${launch_year}, msymbol(s)) ///
	  (scatter pop_logistic year if year >= ${launch_year}, msymbol(t)), ///
	  graphregion(color(white)) ytitle("Population") ylabel(, angle(0))
graph export "Pop_${countyname}_${state}_all.emf", replace
putdocx paragraph, halign(center)
putdocx image "Pop_${countyname}_${state}_all.emf"
putdocx paragraph, halign(center)
putdocx text ("Figure 5.4 "), bold
putdocx text ("Comparison of Population Projections methods and models for ${countyname}, ${state}, ${launch_year}-${target_year}")

putdocx save "PLAN604_PopulationProjection_2018-11-20_${countyname}_${state}_${target_year}.docx", append

/********-*********-*********-*********-*********-*********-*********/
/* Technical Appendix                                               */
/********-*********-*********-*********-*********-*********-*********/
* The following figures and tables may be of importance but are 
* too technical for the main body of the report
putdocx begin
putdocx pagebreak
* Table 3.17 belongs in the appendix
putdocx paragraph, style(Heading2)
putdocx text ("8. Appendix")

* Comparison of all models of population projection
twoway (line pop year, lwidth(thick)) ///
      (scatter sharegrowth year, msymbol(+)) ///
	  (scatter shiftshare year, msymbol(X)) ///
	  (scatter pop_linear year, msymbol(o)) ///
	  (scatter pop_geometric year, msymbol(d)) ///
	  (scatter pop_parabolic year, msymbol(s)) ///
	  (scatter pop_logistic year, msymbol(t)) ///
	  , ///
	  graphregion(color(white)) ytitle("Population") ylabel(, angle(0))
graph export "Pop_${countyname}_${state}_linear.emf", replace
putdocx paragraph, halign(center)
putdocx image "Pop_${countyname}_${state}_linear.emf"
putdocx paragraph, halign(center)
putdocx text ("Figure A.1 "), bold
local first_year = year[1]
putdocx text ("Comparison of Population Projections methods and models for ${countyname}, ${state}, `first_year'-${target_year}")
...
putdocx pagebreak // Table is very large needs to be on a new page
putdocx paragraph, halign(center)
putdocx text ("Table A.1 "), bold
...
putdocx paragraph, halign(left) spacing(before,0) spacing(after,0)
...
save "PLAN604_Wang_vom_Hofe_Chapter_3.4_2018-11-20.dta", replace
